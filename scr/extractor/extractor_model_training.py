...
def main():
    ...
    validation_data_gen = generator2imgsNumData(
        testMeasX, testImgXf, testImgXs, testMeasY, batch_size
    )
...
def load_images():
        ...
        img_spring_npz = np.load(
            open(
                os.path.join(
                    ...
                    f'silh_Xarray{IMG_SIZE_4NN}_SPRING_{gender}_bw.npz'
                ),
                "rb",
            ),
            allow_pickle=True,
        ...
        img_ansuri_npz = np.load(
            open(
                os.path.join(
                    ...
                    f'silh_Xarray{IMG_SIZE_4NN}_ANSURI_{gender}_bw.npz'
                ),
                "rb",
            ),
            allow_pickle=True,
        ...
        img_ansurii_npz = np.load(
            open(
                os.path.join(
                    ...
                    f'silh_Xarray{IMG_SIZE_4NN}_ANSURII_{gender}_bw.npz'
                ),
                "rb",
            ),
            allow_pickle=True,
    ...
    print(f"\nimgX_front.shape = {imgX_front.shape}\n")
...
def createMLP_model(in_MLPlayers=1):
    ...
    mlp_input = Input(shape=trainMeasX.shape[1], name="mlp_input")
...
def generator2imgsNumData(ImgXf, ImgXs, MeasY, batch_size):  # MeasY,
    """
    Generates data for training the model by combining numerical input, front view images, and side view images.

    Args:
        MeasX: Numerical input data
        ImgXf: Front view image data
        ImgXs: Side view image data
        MeasY: Target variable data
        batch_size: Number of samples to yield per batch

    Yields:
        A tuple containing the input data in the form of [numerical input, front view images, side view images]
        and the target variable data.
...
def createCombined_model(MLP_model, CNN_model):
    ...
    input_numca = Input(shape=trainMeasX.shape[1], name="input_numca")
